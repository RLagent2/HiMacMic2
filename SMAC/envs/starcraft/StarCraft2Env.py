...
class StarCraft2Env(MultiAgentEnv):
    ...
    def reward_battle(self):
        ...
        delta_deaths = 0
        delta_ally = 0
        delta_enemy_health = 0
        delta_enemy_shield = 0
        delta_enemy = 0
        ...
        for e_id, e_unit in self.enemies.items():
            if not self.death_tracker_enemy[e_id]:
                prev_health = self.previous_enemy_units[e_id].health
                prev_shield = self.previous_enemy_units[e_id].shield

                if e_unit.health == 0:
                    self.death_tracker_enemy[e_id] = 1
                    delta_deaths += self.reward_death_value
                    delta_enemy_health += prev_health
                    delta_enemy_shield += prev_shield
                    delta_enemy += (prev_health + prev_shield)
                else:
                    delta_enemy_health += max(prev_health - e_unit.health, 0)

                    delta_enemy_shield += max(prev_shield - e_unit.shield, 0)
                    delta_enemy += (prev_health + prev_shield - e_unit.health - e_unit.shield)

        if self.reward_only_positive:
            reward = abs(delta_enemy + delta_deaths)  # shield regeneration (Original setting, the 'abs' is improper)
        else:
            # the original setting: (1) when the enemy's blood or shield is recovered, give allies a '-r';
            #                       (2) when the enemy's recovered blood or shield is attacked by allies, give a '+r';
            #                       (3) the '-r' and '+r' cancel each other out.
            # in summary: the recovered blood or shield has no effect on the overall Return.
    ...
    def get_state(self):
        ...
        # print(self._episode_count, self._episode_steps, ally_state, enemy_state)
