...
class ParallelRunner:
    ...
    def run(self, test_mode=False):
        ...
        while True:
            ...
            if self.t==0:
                tsg_goals = self.tsg_mac.select_goal(self.tsg_batch,t_ep=self.t,t_env=self.t_env, bs=envs_not_terminated, test_mode=test_mode)
                tsg_delta_t = self.tsg_mac.select_skip(self.tsg_batch, t_ep=self.t, t_env=self.t_env,bs=envs_not_terminated, test_mode=test_mode)
                temp_gg = tsg_goals
                temp_tt = tsg_delta_t
                ...
                new_tsg_goals = self.tsg_mac.select_goal(self.tsg_batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated,test_mode=test_mode)
                new_tsg_delta_t = self.tsg_mac.select_skip(self.tsg_batch, t_ep=self.t, t_env=self.t_env,bs=envs_not_terminated,test_mode=test_mode)
            ...
            actions = self.agent_mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated, test_mode=test_mode)
            cpu_actions = actions.unsqueeze(1).to("cpu").numpy()
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                ...
                if not terminated[idx]:
                    ...
                    for i in range(self.args.n_agents):
                        pos_x = data['obs'][i][0:1]
                        pos_y = data['obs'][i][1:2] + self.args.y_length / 2
                        if pos_x<0:
                            pos_x = 0
                        x = pos_x / x_
                        y = pos_y / y_
                    ...
                    #post_transition_data2["hindsight_goals"].append(hind_goal)
                    #post_transition_data3["hindsight_goals"].append(hind_goal)
