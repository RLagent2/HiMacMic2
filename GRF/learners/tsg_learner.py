...
#        avail_actions = batch["avail_actions"]
        ...
        #chosen_action_qvals = chosen_action_qvals1+(chosen_action_qvals2+chosen_action_qvals2_2)/2# Remove the last dim
        ...
        for i in range(self.args.n_agents):
                ...
                if j==0:
                    ...
                    if batch["delta_ts"][:,j,i,:]==0 or j==batch.max_seq_length-1-1:
                        ...
                        if j == batch.max_seq_length - 1 -1:
                            ...
                            final_terminated.append(batch["terminated"][:, j])
                            #if self.args.hindsight:
                            #    final_goals.append(batch["hindsight_goals"][:, j, i])
                            #else:
                            #    final_goals.append(batch["subgoals"][:, j, i])
                            reward = 0
            ...
            for q_values,goal in zip(stack_final_update_chosen_action_qvals_all.squeeze(),stack_final_goals):
                ...
                for id in range(self.args.n_agents):
                    if id==i:
                        ...
                        #S_2d = S.reshape(5,5)
                        #map_2d = pi_exp.reshape(5,5)
                        #l1 = spc.sinkhorn_loss(S_2d, map_2d, epsilon, 5, niter)
                        #l2 = spc.sinkhorn_normalized(S_2d, map_2d, epsilon, 5, niter)
                        #wd_loss = wasserstein_distance(S,pi_exp)
            #loss_success = th.tensor(l2)
            #print("loss",loss_success)
            if self.args.use_cuda:
            ...
            targets = stack_final_update_reward + self.args.gamma * (1 - stack_final_terminated) * mix_target_max_qvals

            targets = build_td_lambda_targets2(stack_final_update_reward, stack_final_terminated, mask, mix_target_max_qvals,
            ...
            if self.args.loss == "cross":
                add_loss = cross_loss
            elif self.args.loss == "wd":
                print("Loss_wrong!")
            else:
                print("Loss_wrong!")
            loss = td_error * 0.1 + add_loss * self.args.count_lmd
    ...
    def cpuu(self):
        self.agent_mac.cpu()
        self.target_agent_mac.cpu()
        self.tsg_mac.cpu()
        self.target_tsg_mac.cpu()
        if self.agent_mixer is not None:
            self.agent_mixer.cpu()
            self.target_agent_mixer.cpu()
        if self.tsg_mixer is not None:
            self.tsg_mixer.cpu()
            self.target_tsg_mixer.cpu()
