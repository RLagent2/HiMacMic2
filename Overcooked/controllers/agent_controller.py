...
class AGENTlevel0MAC:
    ...
    def load_models(self, path):
        self.agent.load_state_dict(th.load("{}/agent_level0.th".format(path), map_location=lambda storage, loc: storage))
    ...
    def _get_input_shape(self, scheme):
        input_shape = scheme["obs"]["vshape"]+np.prod(scheme["subgoals_onehot"]["vshape"])+np.prod(scheme["delta_ts"]["vshape"])
        if self.args.obs_last_action:
